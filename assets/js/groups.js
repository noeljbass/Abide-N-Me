...
export function initGroups() {
  ...
  const pathMatch = location.pathname.match(/\/join\/([A-Za-z0-9-]+)/); if (pathMatch) localStorage.setItem(pendingKey, pathMatch[1]);
  ...
  async function openGroup(group) { selectedGroup = group; list.hidden = true; empty.hidden = true; document.querySelector('[data-join-code-form]').hidden = true; document.querySelector('[data-group-name]').textContent = group.name; document.querySelector('[data-group-description]').textContent = group.description || 'A private Scripture reading group.'; document.querySelector('[data-group-role]').textContent = group.role; detail.hidden = false; try { const data = await api(`groups/members.php?group=${encodeURIComponent(group.id)}`); const members = document.querySelector('[data-group-members]'); members.replaceChildren(...data.members.map((member) => { const item = document.createElement('li'); const avatar = document.createElement('span'); avatar.className = 'avatar avatar-sage'; avatar.textContent = member.name.charAt(0); const name = document.createElement('span'); name.textContent = member.name; const role = document.createElement('span'); role.className = 'status-muted'; role.textContent = member.role; item.append(avatar, name, role); return item; })); } catch (error) { showMessage(document.querySelector('[data-group-detail-message]'), error.message); } }
  ...
  document.querySelector('[data-create-group]').addEventListener('submit', async (event) => { event.preventDefault(); const message = document.querySelector('[data-group-form-message]'); try { const { group } = await api('groups/index.php', { method: 'POST', body: Object.fromEntries(new FormData(event.currentTarget)) }); event.currentTarget.reset(); event.currentTarget.closest('details').open = false; await loadGroups(); openGroup(group); } catch (error) { showMessage(message, error.message); } });
  ...
  document.querySelector('[data-create-invite]').addEventListener('click', async () => { try { const { invite } = await api('groups/invite.php', { method: 'POST', body: { group_id: selectedGroup.id, role: 'member', expires_in_days: 30 } }); const url = `${location.origin}${location.pathname.replace(/\/join\/.*$/, '/').replace(/index\.html$/, '')}join/${invite.code}`; document.querySelector('[data-invite-link]').value = url; document.querySelector('[data-invite-result]').hidden = false; } catch (error) { showMessage(document.querySelector('[data-group-detail-message]'), error.message); } });
