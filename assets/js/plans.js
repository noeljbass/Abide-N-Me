...
export function initPlans() {
  ...
  function syncMode(){
    const automatic=new FormData(form).get('mode')==='automatic';
    const automaticFields=form.querySelector('[data-automatic-fields]');
    const manualFields=form.querySelector('[data-manual-fields]');
    automaticFields.hidden=!automatic;
    manualFields.hidden=automatic;
    automaticFields.querySelectorAll('input, select, textarea, button').forEach(control=>{control.disabled=!automatic;});
    manualFields.querySelectorAll('input, select, textarea, button').forEach(control=>{control.disabled=automatic;});
  }
  syncMode();
  ...
  form.addEventListener('submit',async event=>{event.preventDefault();show('');const mode=new FormData(form).get('mode');const payload={group_id:groups.value,name:form.elements.name.value,description:form.elements.description.value,mode,start_date:form.elements.start_date.value,timezone:form.elements.timezone.value,translation:'DRA1899'};if(mode==='automatic'){payload.books=[...books.querySelectorAll('input:checked')].map(input=>input.value);payload.duration_days=Number(form.elements.duration_days.value);payload.weekdays=[...form.querySelectorAll('[data-weekdays] input:checked')].map(input=>Number(input.value));}else{payload.days=[...manual.querySelectorAll('.manual-day')].map(day=>({title:day.querySelector('[data-day-title]').value,passages:day.querySelector('[data-day-passages]').value,note:day.querySelector('[data-day-note]').value,question:day.querySelector('[data-day-question]').value}));}const submit=form.querySelector('[type="submit"]');submit.disabled=true;try{await api('plans/index.php',{method:'POST',body:payload});show('Plan created and assigned to your group.',true);form.reset();form.elements.start_date.value=new Date().toISOString().slice(0,10);manual.replaceChildren();dayCount=0;addDay();syncMode();document.querySelector('[data-plan-builder]').open=false;await load();}catch(error){show(error.message);}finally{submit.disabled=false;}});
