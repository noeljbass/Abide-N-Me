...
The reader also offers browser-native text-to-speech when no recorded-audio
version is active. This reads the currently displayed chapter with the voice
installed on the listener's device and does not require Bible Brain, an API key,
or a server-side audio provider. Voice availability and quality therefore vary
